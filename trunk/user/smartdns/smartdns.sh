...
Change_iptable() {
    ...
    start)
        if [ "$snds_redirected" != 2 ] && [ "$snds_redirect" = 2 ]; then
            statu=1
            logger -t "SmartDNS" "将会重定向 xxx.xxx.xxx:53 至 $IPS4:$sdns_port"
        fi
        ;;
    reset)
...
Start_smartdns() {
    # 【】
    : >"$smartdns_Ini" # clear  smartdns_Ini
    [ "$sdns_enable" -eq 0 ] && nvram set sdns_enable=1 && sdns_enable=1
    [ $(pidof smartdns | awk '{ print $1 }')x != x ] && killall -9 smartdns >/dev/null 2>&1 # kill other smartdns
    Change_dnsmasq
    Change_adbyby
    ...
    if [ "$sdns_coredump" = "1" ]; then
        args="$args -S" # ？ ？   -S            ignore segment fault signal.
    fi
...
Main() {
    ...
    start)
        if [ ! -s "$smartdns_Ini" ]; then
            logger -t "SmartDNS" "smartdns_conf.ini 为空 ，启动．．．"
        fi
        Check_ss
        Start_smartdns
